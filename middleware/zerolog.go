...
// ZeroLog returns a middleware that logs commands execution.
func ZeroLog() commandbus.MiddlewareFunc {
	return ZeroLogWithConfig(DefaultZeroLogConfig)
}

// ZeroLogWithConfig returns a ZeroLog middleware with config.
// See: `Zerolog()`.
func ZeroLogWithConfig(cfg ZeroLogConfig) commandbus.MiddlewareFunc {
	// Defaults
